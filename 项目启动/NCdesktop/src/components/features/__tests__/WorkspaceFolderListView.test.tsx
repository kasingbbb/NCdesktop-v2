 ...
 *   - 右键 ai_organized 行：仅「在 Finder 中显示」可点，其余灰显（点击灰显项不触发 handler）
 *   - 右键 __ROOT__ 行：仅「在 Finder 中显示」（不含重命名 / 删除条目）
...
describe("WorkspaceFolderListView — 右键菜单形态", () => {
  it("root 行：显示 重命名 / 移到废纸篓 / 在 Finder 中显示", () => {
    renderView();
  ...
  it("__ROOT__ 行：仅显示「在 Finder 中显示」，不含重命名/删除", () => {
    const onReveal = vi.fn();
    renderView({ onReveal });
