...
export function DropzoneExpanded() {
  ...
  return (
    <div
      className="glass-card-elevated flex flex-col w-full h-full min-h-0 overflow-hidden pointer-events-auto"
      style={{
        borderRadius: 22,
        padding: "var(--space-3)",
    ...
    >
      ...
      <div className="flex items-center justify-between mb-[var(--space-2)]">
        <span className="text-[var(--text-sm)] font-medium" style={{ color: "var(--text-primary)" }}>最近导入</span>
        <button
      ...
      {recentItems.length === 0 ? (
        <div
          ...
          style={{ color: "var(--text-tertiary)" }}
        >
          <span className="text-[var(--text-xs)]">暂无导入记录</span>
        </div>
      ) : (
          ...
          {recentItems.slice(0, 5).map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[var(--text-xs)] truncate" style={{ color: "var(--text-primary)" }}>{item.fileName}</div>
                <div className="text-[10px] line-clamp-2" style={{ color: "var(--text-tertiary)" }}>
                  {item.detail ??
                    (item.status === "done"
                      ? "已入库"
